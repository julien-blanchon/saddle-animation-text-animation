...
pub struct TextAnimationLabE2EPlugin;

impl Plugin for TextAnimationLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(saddle_bevy_e2e::E2EPlugin);
        app.configure_sets(Update, E2ESet.before(TextAnimationSystems::DetectChanges));
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[text_animation_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
