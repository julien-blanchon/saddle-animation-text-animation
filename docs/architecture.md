...
2. Rebuild grapheme and reveal-unit caches only when needed, including layout-driven rebuilds when `TextLayoutInfo` changes.
3. Advance playback time according to scaled or unscaled time.
4. Resolve visible reveal units, reduced-motion state, and effect-local time in `EvaluateEffects`.
5. Apply final positions, colors, and alpha to overlay glyph entities in `ApplyOutput` once Bevy has published the latest text layout for the frame.
6. Emit start, completion, loop, and checkpoint messages.
...
- per-glyph scale and rotation are not yet part of the public effect surface
